...
class LOLRank:
    ...
    @commands.group(pass_context = True, description='List of league of legends commands.')
    async def lol(self, ctx):
        if (ctx.invoked_subcommand is None):
            await self.bot.say('Incorrect random subcommand passed.')

    @lol.group(name='rank', pass_context=True, description='Displays ranked data of player.')
    ...
    @display_ranked_data.command(name='', hidden=True)
